...
BasbosaRest.prototype = {
          ...
          Basbosa('App')[verb](url, function(req, res, next) {
            ...
            boundFunction(req, res, function() {
              if (self.options('defaultTemplates') !== 'json') {
                ...
                res.locals.view = res.locals.view || self.options('templates') + action + '_' + self.options('defaultTemplates');
                res.locals.resourceRoot = req.rest.resourceRoot;
              } else {
                res.locals.view = res.locals.view || self.options('templates') + 'json';
              }
              next();
            });
          });
  ...
  read : function(req, res, next) {
    var
      ...
      qOptions = B('_').extend({limit : 20, sort : [['_id', 'desc']]}, q.qOptions);
    ...
    if (req.method === 'GET' && !req.rest.query.data) {
      B('AutoModels').getModel(req.rest.modelName, function(err, model) {
        var
          q = req.rest.query,
          fields = B('_').extend({}, q.fields),
          qOptions = B('_').extend({limit : 1}, q.qOptions);

        model.find({_id : new (B('ObjectId'))(req.param('_id'))}, fields, qOptions, function(err, results) {
